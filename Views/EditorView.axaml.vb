...
Namespace Views
    ...
    Public Class EditorView
        ...
        Private Const CornerHandleTolerance As Double = 16.0
        ...
        ' Ein Bild aus dem Filmstreifen auf die Leinwand ziehen (wird dort zur Bildebene). Der Druck
        ' merkt sich das Bild nur; gezogen wird erst ab einer Mindeststrecke, gewechselt erst beim
        ' Loslassen ohne Zug. DIESELBE Strecke trennt beides, sonst gaebe es einen Bereich, in dem
        ' weder gezogen noch gewechselt wird.
        Private Const FilmstripDragThreshold As Double = 6.0
        Private _filmstripDragItem As ImageItem
        Private _filmstripDragStart As Avalonia.Point
        Private _filmstripDragArgs As PointerPressedEventArgs
        Private _filmstripDragActive As Boolean = False
        ...
        Private Shared Function StageItem(vm As EditorViewModel) As ImageItem
            ...
            If String.IsNullOrWhiteSpace(path) OrElse Not IO.File.Exists(path) Then Return Nothing
        ...
        Private Sub UpdateSliderLayout()
            ...
            If EditorViewModel.NeedsRefit(_zoomInitialized, vm.ActiveZoomPreset = ZoomPresetMode.Fit,
        ...
        Private Sub OnSliderPointerPressed(sender As Object, e As PointerPressedEventArgs)
            ...
            If vm IsNot Nothing Then
                ...
                If imageRect.Width > 0 AndAlso imageRect.Height > 0 AndAlso
                   ...
                   Not BrushCircleTouchesImage(e.GetPosition(canvas), imageRect, vm) Then
                    ...
                    Dim startsGridPointOutside = ShowsGrid(vm) AndAlso
                                                   HitsGridPoint(vm, canvas, e.GetPosition(canvas))
            ...
            ' Verzerrung nur hier.
            If vm IsNot Nothing AndAlso vm.ShowsObjectCorners Then
                Dim oRect = GetDisplayedImageRect(canvas, vm)
                If oRect.Width > 0 AndAlso oRect.Height > 0 Then
                    Dim oPos = e.GetPosition(canvas)
                    If vm.TryBeginObjectCornerDrag((oPos.X - oRect.Left) / oRect.Width * 100.0,
                                                 (oPos.Y - oRect.Top) / oRect.Height * 100.0,
                                                 CornerHandleTolerance / oRect.Width * 100.0,
                                                 CornerHandleTolerance / oRect.Height * 100.0) Then
                        _isObjectCornerDragging = True
                        e.Pointer.Capture(canvas)
                        e.Handled = True
                        Return
                    End If
            ...
            If ShowsPerspective(vm) Then
                ...
                If eckRect.Width > 0 AndAlso eckRect.Height > 0 Then
                    ...
                    If vm.TryBeginPerspectiveCornerDrag((eckPos.X - eckRect.Left) / eckRect.Width * 100.0,
                                                        ...
                                                        CornerHandleTolerance / eckRect.Width * 100.0,
                                                        CornerHandleTolerance / eckRect.Height * 100.0) Then
            ...
            If ShowsGrid(vm) Then
                Dim gridRect = GetDisplayedImageRect(canvas, vm)
                If gridRect.Width > 0 AndAlso gridRect.Height > 0 Then
                    ' NICHT klemmen: ein Randpunkt sitzt auf der Kante, und die aeussere Haelfte
                    ' seines Fangbereichs liegt daneben.
                    Dim gridPos = e.GetPosition(canvas)
                    ...
                    ' Haelfte gar nicht.
                    Const gridSlopPixels As Double = CornerHandleTolerance
                    If vm.TryBeginWarpDrag((gridPos.X - gridRect.Left) / gridRect.Width * 100.0,
                                           (gridPos.Y - gridRect.Top) / gridRect.Height * 100.0,
            ...
            If ShowsLines(vm) Then
                Dim linesRect = GetDisplayedImageRect(canvas, vm)
                If linesRect.Width > 0 AndAlso linesRect.Height > 0 Then
                    ...
                    Dim lx = (lPos.X - linesRect.Left) / linesRect.Width * 100.0
                    Dim ly = (lPos.Y - linesRect.Top) / linesRect.Height * 100.0
                    Dim slopX = CornerHandleTolerance / linesRect.Width * 100.0
                    Dim slopY = CornerHandleTolerance / linesRect.Height * 100.0
        ...
        Private Sub OnSliderPointerMoved(sender As Object, e As PointerEventArgs)
            ...
            If _isObjectCornerDragging Then
                ...
                oVm.UpdateObjectCornerDrag((oPos.X - oRect.Left) / oRect.Width * 100.0,
                                         (oPos.Y - oRect.Top) / oRect.Height * 100.0)
                UpdateSliderLayout()
                e.Handled = True
                Return
        ...
        Private Sub FinishPointerGesture(e As PointerEventArgs)
            ...
            End If
            If _isObjectCornerDragging Then
                TryCast(DataContext, EditorViewModel)?.EndObjectCornerDrag()
                _isObjectCornerDragging = False
                UpdateSliderLayout()
            End If
            If _isGradientDragging Then
        ...
        Private Sub UpdateGuideDrag(canvasPosition As Avalonia.Point)
            ...
            Dim source = If(aufSchirmSenkrecht,
        ...
        Private Function HitsWarpCorner(vm As EditorViewModel, canvas As Canvas,
            ...
            For i = 0 To 3
                ...
                Dim y = rect.Top + corners(i * 2 + 1) / 100.0 * rect.Height
        ...
        Private Function HitsGridPoint(vm As EditorViewModel, canvas As Canvas,
                                           position As Avalonia.Point) As Boolean
            If vm Is Nothing OrElse canvas Is Nothing Then Return False
            Dim rect = GetDisplayedImageRect(canvas, vm)
            If rect.Width <= 0 OrElse rect.Height <= 0 Then Return False
            ...
            Dim count = (CInt(g(0)) + 1) * (CInt(g(1)) + 1)
            For i = 0 To count - 1
        ...
        Private Shared Sub HalteSeitenverhaeltnis(modus As CropDragMode, ausgang As Avalonia.Rect,
            ...
            Select Case modus
                ...
                Case CropDragMode.Top, CropDragMode.Bottom
                    widthLeads = False
                Case Else
                    widthLeads = Math.Abs(width - ausgang.Width) >= Math.Abs(height - ausgang.Height)
            End Select
            ...
            Dim toLeft = (modus = CropDragMode.Left OrElse modus = CropDragMode.TopLeft OrElse
                             modus = CropDragMode.BottomLeft)
            Dim toRight = (modus = CropDragMode.Right OrElse modus = CropDragMode.TopRight OrElse
                              modus = CropDragMode.BottomRight)
            Dim toTop = (modus = CropDragMode.Top OrElse modus = CropDragMode.TopLeft OrElse
                            modus = CropDragMode.TopRight)
            Dim toBottom = (modus = CropDragMode.Bottom OrElse modus = CropDragMode.BottomLeft OrElse
                             modus = CropDragMode.BottomRight)

            ' Wie weit reicht das Rechteck vom festen Punkt aus hoechstens ins Bild?
            Dim platzX = If(toLeft, ankerX - imageRect.Left,
                         If(toRight, imageRect.Right - ankerX,
                            Math.Min(ankerX - imageRect.Left, imageRect.Right - ankerX) * 2.0))
            Dim platzY = If(toTop, ankerY - imageRect.Top,
                         If(toBottom, imageRect.Bottom - ankerY,
                            Math.Min(ankerY - imageRect.Top, imageRect.Bottom - ankerY) * 2.0))
            If platzX > 0 AndAlso width > platzX Then
                width = platzX
                height = width / target
            End If
            If platzY > 0 AndAlso height > platzY Then
                height = platzY
                width = height * target
            End If
            ...
            If toLeft Then
                right = ankerX : left = ankerX - width
            ElseIf toRight Then
                left = ankerX : right = ankerX + width
            Else
                left = ankerX - width / 2.0 : right = ankerX + width / 2.0
            End If
            If toTop Then
                bottom = ankerY : top = ankerY - height
            ElseIf toBottom Then
                top = ankerY : bottom = ankerY + height
            Else
                top = ankerY - height / 2.0 : bottom = ankerY + height / 2.0
            End If
        End Sub
        ...
        ''' <summary>Die Lage des Auswahlrahmens: NUR die Drehung.
        '''
        ''' DIE SPIEGELUNG GEHOERT AUSDRUECKLICH NICHT HINEIN, und das ist gemessen: der Rahmen ist
        ''' ein achsenparalleles Rechteck, eine Spiegelung um seine eigene Mitte laesst ihn also
        ''' unveraendert. Alles, was sie bewegt, sind seine GRIFFE - und da sitzt der Dreh-Anfasser
        ''' als einziger unsymmetrisch, oben ueber der Mitte. Ein gespiegelter Rahmen klappte ihn
        ''' nach unten, waehrend die Trefferpruefung (GetTextDragMode) ihn weiter oben suchte: er
        ''' war zu sehen und nicht zu fassen (Befund an einem Textobjekt mit eigener senkrechter
        ''' Spiegelung).
        '''
        ''' Wer das aendern will, muss GetTextDragMode mitaendern - und dabei bedenken, dass die
        ''' Groessengriffe NICHT mitgespiegelt werden duerfen: das Rechteck liegt im Anzeigeraum,
        ''' wer seine sichtbar obere Kante zieht, meint dort auch die obere.</summary>
        Private Shared Function BuildSelectionFrameTransform(rotationDegrees As Double) As Transform
        ...
        Private Sub PositionTextOverlayFromViewModel(ix As Double, iy As Double, iw As Double, ih As Double, scale As Double)
            ...
            overlay.RenderTransform = BuildSelectionFrameTransform(If(vm.HasMultiAnnotationSelection, 0.0, vm.AnnotationRotation))
            overlay.IsVisible = True
            ...
            If frame IsNot Nothing Then
                Dim frameColor = ParseAvaloniaColor(vm.AnnotationStrokeColor, Colors.White)
                frame.Stroke = New SolidColorBrush(frameColor)
        ...
        Private Shared Function NoHandlesWhileWarping(vm As EditorViewModel, mode As TextDragMode) As TextDragMode
            ...
            If vm IsNot Nothing AndAlso vm.WarpsTheObject Then Return TextDragMode.None
            Return mode
        End Function
        ...
        Public Sub OnTextOverlayPointerMoved(sender As Object, e As PointerEventArgs)
            ...
            If _textDragMode = TextDragMode.Rotate Then
                ...
                If vm.HasMultiAnnotationSelection Then
                    ...
                    overlay.RenderTransformOrigin = New RelativePoint(0.5, 0.5, RelativeUnit.Relative)
                    overlay.RenderTransform = BuildSelectionFrameTransform(_textRotateBoxAngle)
                    e.Handled = True
                    Return
                End If
                ...
                overlay.RenderTransform = BuildSelectionFrameTransform(newRotation)
                e.Handled = True
                Return
            End If
        ...
        Private Sub UpdateTextSizeBadge(rect As Avalonia.Rect, imageRect As Avalonia.Rect, vm As EditorViewModel)
            ...
            If badge IsNot Nothing Then
                ...
                ' Die Anzeige liegt IM Rahmen und traegt dessen Drehung mit; sie nimmt sie zurueck,
                ' damit die Zahl aufrecht steht.
        ...
        Public Async Sub OnFilmstripPointerMoved(sender As Object, e As PointerEventArgs)
            ...
            If e.Pointer.Type = PointerType.Touch Then Return
            Dim delta = e.GetPosition(Me) - _filmstripDragStart
            If Math.Abs(delta.X) < FilmstripDragThreshold AndAlso Math.Abs(delta.Y) < FilmstripDragThreshold Then Return

            Dim item = _filmstripDragItem
            Dim pressedArgs = _filmstripDragArgs
            _filmstripDragItem = Nothing
            _filmstripDragArgs = Nothing
            If item Is Nothing OrElse pressedArgs Is Nothing Then Return
            If item.IsRemoteAsset Then Return
        ...
        Public Sub OnGlobalPointerReleased(sender As Object, e As PointerReleasedEventArgs)
            ...
            End If
            ' Losgelassen ausserhalb des Filmstreifens: der gemerkte Druck darf nicht liegen
            ' bleiben, sonst wechselte das naechste Loslassen zum falschen Bild.
            _filmstripDragItem = Nothing
            _filmstripDragArgs = Nothing
        End Sub
