...
- Paint tool: brush, eraser, blur, and clone stamp, each with its own remembered brush settings (size/radius, hardness or strength, opacity, and flow); brush, eraser, and blur settings are visible as soon as the tool is selected, and the stamp takes its source from an Alt+click and keeps the offset for the whole stroke, with a dashed ring marking the sampling point
- Move tool for selecting and repositioning existing objects without starting a new image selection; the toolbar keeps the active tool highlighted while also accenting the type of the selected object
- Selection tool with four modes (rectangle, ellipse, freehand lasso, magic wand): draw or click a selection on the image, then copy it into a new movable object (also via Ctrl+C/Ctrl+V, repeatable paste) or fill it — copied/filled selections switch straight into the move tool so the new object is immediately visible and draggable; the rectangle supports a solid color or linear/radial gradient (direction/invert), irregular shapes are filled with a solid color; the magic wand selects a contiguous color area with an adjustable tolerance
...
- Per-object properties: fill (solid or gradient), stroke color/width (text outlines up to 100 px), opacity, blend mode, rotation, position/size, anchor handling, plus separate shadow and glow controls (color, offset, blur, strength, corner radius where applicable) — edited live directly on the canvas or via the sliders
...
- Side panel with three tabs: the active tool, the object list (reorder front/back, duplicate, show/hide, delete; drag-handles for move/resize/rotate on canvas), and a running history of the applied steps; selection-derived objects behave like normal objects once created
...
**Immich integration**
- Connect a self-hosted [Immich](https://immich.app/) server (server URL + API key, with a connection test in Settings → Immich); a dedicated *Immich* section appears in the gallery's navigation panel with an *All photos* entry and one node per album
- Browse albums (or the whole timeline) with thumbnails served from Immich and cached locally, loaded in pages so photos appear as they arrive; open photos fullscreen with the **whole album in the filmstrip** (originals downloaded on demand, videos play inline). Hidden assets — such as the video part of a motion photo, which Immich stores separately and has no preview for — are left out, as they are in Immich's own interface
...
- Create, rename, and delete Immich albums, and upload local images/videos to Immich (or straight into an album), from the Immich section's context menu — deleting an album removes only the album, the photos stay in Immich, and the entry appears only when *Allow deleting in Immich* is on
...
- Edit Immich photos in the editor in **save-as-only** mode by default — the original is never overwritten; save the result as a new local file in a selectable folder, or (via the save-as *destination* switch) upload it back to Immich as a new asset, into the source album when applicable
- **Update existing assets** (Settings → Immich, off by default): editing an Immich photo replaces that photo instead of adding a copy — *Save* in the editor becomes available for Immich photos, and batch operations update the photos in place. Albums, favorite, stack, shared links, description, rating, keywords and the capture date are carried over, and the original is moved to the Immich trash. (Immich v3 removed the "replace original" endpoint, so FerrumPix uploads the edit, copies the asset's links and metadata over, and trashes the old asset; on older servers the single-call endpoint is used, which keeps the asset ID.) On v3 the asset ID necessarily changes, so face/person assignments are re-derived by Immich and album comments do not follow the photo
- Batch convert, resize, filter, and watermark operations on Immich photos create new Immich assets (or update the source assets, with *Update existing assets* on); when started from an album, new assets are assigned back to that album, and FerrumPix waits briefly for Immich thumbnails before refreshing
- **Delete Immich photos** from the gallery and viewer — off by default and guarded by *Allow deleting in Immich* in Settings, since this removes photos from the server, not just from a local view; deleted photos land in the Immich trash unless *Delete permanently* is switched on as well
...
- Current Immich limits: the editor still initializes its rating/favorite/tag fields from the local temp/catalog view when opening an Immich photo, but changes are written back to Immich; renaming individual Immich assets is not implemented yet
