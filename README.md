...
To be transparent: yes, I use AI to support my development workflow. AI helps me also with translation. However, it still requires a massive amount of manual work, architecture planning, and debugging. I am currently investing a lot of time. A ton of passion, unique ideas, and genuine hard work went into this project.
...
**With a model file installed** (see below), four more things become available: clicking an object to select it, selecting by distance from the camera, removing something so the background is continued through the gap (on the photo or inside a marked layer), and denoising a photo with a model, which can measure the noise itself and pick the strength. Applying a filter to a selection can denoise each picture along the way and leaves clean ones alone.
