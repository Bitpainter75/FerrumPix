...
Namespace Controls
    ...
    Public Class LineWarpOverlayControl
        ...
        Public Overrides Sub Render(context As DrawingContext)
            ...
            Dim anzahl = CInt(g(0))
            If anzahl < 1 OrElse g.Length < 1 + anzahl * 8 Then Return
            ...
            For i = 0 To anzahl - 1
                ...
                Dim bewegt = Math.Abs(za.X - qa.X) > 0.5 OrElse Math.Abs(za.Y - qa.Y) > 0.5 OrElse
                ...
                If bewegt Then
                    ...
                    context.DrawLine(quelllinie, qa, qb)
                    context.DrawLine(verbindung, qa, za)
                    context.DrawLine(verbindung, qb, zb)
                End If

                context.DrawLine(schatten, za, zb)
                context.DrawLine(ziellinie, za, zb)
                context.DrawEllipse(fuellung, rand, za, GriffRadius, GriffRadius)
                context.DrawEllipse(fuellung, rand, zb, GriffRadius, GriffRadius)
            Next
        End Sub
    End Class
